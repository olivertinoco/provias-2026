...
BEGIN
	...
	BEGIN
		SELECT
		top 1 @vDias=CASE WHEN  COALESCE(EDOD.FechaDestinoRecepciona,'')='' THEN CASE WHEN DATEDIFF(DAY,CONVERT(DATE, EDO.FechaOrigen),GETDATE())<0 then 0 ELSE DATEDIFF(DAY,CONVERT(DATE, EDO.FechaOrigen),GETDATE()) END ELSE 0 END
		FROM
		...
		AND Ed.IdExpediente=@pIdExpediente AND EDOD.IdCargoDestino =@pIdCargo AND EDOD.IdEmpresaDestino=@pIdEmpresa and edod.IdAreaDestino=@pIdArea and EDOD.IdPersonaDestino=@pIdPersona
	END
