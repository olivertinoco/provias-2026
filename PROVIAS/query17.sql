...
select top 0
cast(null as int) pIdArea,
cast(null as int) pIdUsuarioAuditoria,
cast(null as varchar(50)) collate database_default pCampoOrdenado,
cast(null as varchar(4)) collate database_default pTipoOrdenacion,
cast(null as int) pNumeroPagina,
cast(null as int) pDimensionPagina,
cast(null as varchar(100)) collate database_default  pBusquedaGeneral into #tmp001_params

insert into #tmp001_params
select @pIdArea,
    @pIdUsuarioAuditoria,
    rtrim(ltrim(isnull(@pCampoOrdenado,''))),
    rtrim(ltrim(isnull(@pTipoOrdenacion,''))),
    @pNumeroPagina,
    @pDimensionPagina,
    rtrim(ltrim(isnull(@pBusquedaGeneral,'')))


select*from #tmp001_params
-- AS
-- 	BEGIN TRY
		...
		--
select @pBusquedaGeneralfText
= concat('"',@pBusquedaGeneral,'*"')
				...
				FROM
					(
						...
						FROM
							...
							ED.IdExpediente=E.IdExpediente AND ED.EstadoAuditoria=E.EstadoAuditoria AND
							ED.IdEmpresaEmisor=0 AND E.IdCatalogoSituacionExpediente=63
		...
		SELECT
			...
			case when COALESCE(E.RazonSocialNombreRemitente,'')='' then COALESCE(E.NombreCompletoCreador,'')
