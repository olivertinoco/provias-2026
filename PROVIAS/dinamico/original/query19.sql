CREATE PROCEDURE [Tramite].[paObtenerExpedienteDocumentoOrigenDestino]
		@pIdExpedienteDocumentoOrigenDestino INT

		AS
		BEGIN
		BEGIN TRY
		--CONTABILIZAR PLAZO PARA TIPOS DE TRAMITE
			...
			FROM
			Tramite.ExpedienteDocumentoOrigenDestino EDOD
		...
		END TRY
			BEGIN CATCH
				...
				SELECT @ERROR_NUMBER=ERROR_NUMBER() , @ERROR_SEVERITY=ERROR_SEVERITY() , @ERROR_STATE=ERROR_STATE() , @ERROR_PROCEDURE='Tramite.paObtenerExpedienteDocumentoOrigenDestino',@ERROR_LINE=ERROR_LINE(),@ERROR_MESSAGE=ERROR_MESSAGE()
