CREATE PROCEDURE [Tramite].[paListarPendienteFirmaDigitalEspecialistaV1]
@pIdArea int,
@pIdCargo int,
@pIdPersona int,
...
AS
 SET NOCOUNT ON;
	...
	SET LANGUAGE 'SPANISH'
			...
			+' WHERE 1=1 AND EDF.FlagFirmado=0 AND FgEsObligatorioFirmaDigital=1 '
			+@Filtros
	...
	BEGIN CATCH
			...
			SELECT @ERROR_NUMBER=ERROR_NUMBER() , @ERROR_SEVERITY=ERROR_SEVERITY() , @ERROR_STATE=ERROR_STATE() , @ERROR_PROCEDURE='Tramite.paListarPendienteFirmaDigitalEspecialista',@ERROR_LINE=ERROR_LINE(),@ERROR_MESSAGE=ERROR_MESSAGE()
