...
set language spanish
	...
	LEFT JOIN Tramite.ExpedienteSeguimiento ES ON ES.IdExpediente= E.IdExpediente AND ES.EstadoAuditoria=1 AND ES.IdEmpresa=2 AND ES.IdPersona='+CONVERT(VARCHAR,@pIdPersona)+'
	...
	WHERE EDOD.EstadoAuditoria=1 AND ED.IdPersonaEmisor=' + CONVERT(VARCHAR,@pIdPersona) + ')X WHERE 1=1 ' + @vFiltroDocumento + @Filtros
	...
	LEFT JOIN General.Area A ON A.IdArea=ED.IdAreaEmisor
	LEFT JOIN General.Empresa EM ON EM.IdEmpresa=ED.IdEmpresaEmisor
	WHERE EDOD.EstadoAuditoria=1 AND ED.IdPersonaEmisor='+CONVERT(VARCHAR,@pIdPersona)+')X WHERE 1=1 '+ @vFiltroDocumento + @Filtros + @Orden + @Offset + @Fetch
