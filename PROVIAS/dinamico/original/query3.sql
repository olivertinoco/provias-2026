CREATE PROCEDURE [Tramite].[paListarMisDocumentosGeneradosEspecialistaV1]
       @pIdPersona int,
...
AS
	   ...
	   set language 'spanish'
			...
			IF COALESCE(@pFechaDocumento,'')<>'' BEGIN	SET @vFiltroDocumento =@vFiltroDocumento+' AND convert(date,X.NFechaDocumento) between '''+@vFechaInicial+''' AND '''+@vFechaFinal+'''' END
             ...
             IF COALESCE(@pBusquedaGeneral,'')<>'' SET @Filtros =' AND (X.NombreExpediente LIKE ''%'+@pBusquedaGeneral +'%'' or X.AsuntoDocumento LIKE ''%'+@pBusquedaGeneral +'%'' or X.NumeroDocumento LIKE ''%'+@pBusquedaGeneral +'%'' or X.ObservacionesDocumento LIKE ''%'+@pBusquedaGeneral +'%'' or X.NFechaDocumento LIKE ''%'+@pBusquedaGeneral +'%'')'
             ...
             SET @ConsultaTotal = N'SELECT @vpTotalRegistros = count(*)
				...
				WHERE EDOD.EstadoAuditoria=1 AND ED.IdPersonaEmisor='+CONVERT(VARCHAR,@pIdPersona)+')X WHERE 1=1 '+@vFiltroDocumento
                    +@Filtros
