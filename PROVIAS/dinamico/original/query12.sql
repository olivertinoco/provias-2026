...
SET LANGUAGE SPANISH
	...
	ELSE
		BEGIN
    		...
    		BEGIN
				...
				WHERE ED.IdExpediente=@pIdExpediente AND
				(
    ...
    WHERE EDOD.EstadoAuditoria=1 AND E.IdExpediente=' + CONVERT(VARCHAR,@pIdExpediente) + @Filtros + @vCondicionVinculado
    ...
    LEFT JOIN General.Persona PA ON PA.IdPersona=EDOD.IdPersonaDestinoAtencion
	LEFT JOIN Courrier.Envio EE ON EE.IdExpedienteDocumentoOrigenDestino =	EDOD.IdExpedienteDocumentoOrigenDestino AND EE.EstadoAuditoria=1 AND FgEntregado=0
    WHERE EDOD.EstadoAuditoria=1 AND E.IdExpediente=' + CONVERT(VARCHAR,@pIdExpediente) + @Filtros + @vCondicionVinculado + @Orden + @Offset + @Fetch
