...
AS
	   ...
	   SET LANGUAGE 'SPANISH'
			...
			SELECT @vIdTipoFormulario=COUNT(IdTipoFormulario) FROM Tramite.PermisoVisualizacionDocumentos WHERE EstadoAuditoria=1 and IdPersona=@vIdPersonaActual AND IdTipoFormulario=3 and convert(date, GETDATE() )between convert(date,FechaInicioPersmiso) and convert(date,FechaFinPersmiso)

			IF @vIdTipoFormulario>0-- in(select TOP 1 IdUsuario from Tramite.PermisoVisualizacionDocumentos where IdTipoFormulario=3 and convert(date, GETDATE() )between convert(date,FechaInicioPersmiso) and convert(date,FechaFinPersmiso)) --Habilitar usuarios
			BEGIN
				SET @vSiPariticipo=1
			END
			ELSE
				...
				SELECT @vIdCargoJefeEsMio=IdCargo, @vIdAreaJefeEsMio=IdArea,@vIdEmpresaJefeEsMio=IdEmpresa FROM RecursoHumano.visPersonaJefe where IdArea=@pIdArea AND IdPersona = @vIdPersonaActual

				SET @vSiPariticipo=(select COUNT(ED.IdPersonaEmisor) FROM
						...
						WHERE ED.IdExpediente=@pIdExpediente AND --( EDOD.IdPersonaDestino = @vIdPersonaActual OR EDO.IdPersonaOrigen=@vIdPersonaActual))
						(
						(EDOD.IdCargoDestino = @vIdCargoJefeEsMio and EDOD.IdAreaDestino=@vIdAreaJefeEsMio) OR (EDO.IdCargoOrigen=@vIdCargoJefeEsMio or EDO.IdAreaOrigen=@vIdAreaJefeEsMio))
						)
			...
			-------------------------------------------------------------------------------------------------------------------------------------
             ...
             SET @ConsultaTotal = N'SELECT @vpTotalRegistros = count(*)
                   FROM Tramite.Expediente E
                    ...
                    WHERE EDOD.EstadoAuditoria=1 AND E.IdExpediente='+CONVERT(VARCHAR,@pIdExpediente)
                    +@Filtros
					+@vCondicionVinculado
             ...
             SELECT case when ED.FgEnEsperaFirmaDigital=1 and Ver.doc=0 then 0 else '+
					CONVERT(VARCHAR,@vSiPariticipo) +' end SiPariticipo,
             EDO.EsVinculado,
             E.ExpedienteAnulado,
                    E.IdExpediente,
					...
					ED.FlagParaDespacho,
					COALESCE(ED.IdExpedienteVirtual,0) IdExpedienteVirtual
                    FROM Tramite.Expediente E
                    INNER JOIN Tramite.ExpedienteDocumento ED ON ED.IdExpediente=E.IdExpediente
                    INNER JOIN Tramite.ExpedienteDocumentoOrigen EDO ON EDO.IdExpedienteDocumento=ED.IdExpedienteDocumento AND ED.EstadoAuditoria=1
                    INNER JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD ON EDOD.IdExpedienteDocumentoOrigen=EDO.IdExpedienteDocumentoOrigen  AND EDO.EstadoAuditoria=1
       ...
       BEGIN CATCH
                    ...
                    SELECT @ERROR_NUMBER=ERROR_NUMBER() , @ERROR_SEVERITY=ERROR_SEVERITY() , @ERROR_STATE=ERROR_STATE() , @ERROR_PROCEDURE='Tramite.paListarDetalleBusquedaExpedienteGeneral',@ERROR_LINE=ERROR_LINE(),@ERROR_MESSAGE=ERROR_MESSAGE()
...
--EXEC Tramite.paListarDetalleBusquedaExpedienteGeneral 15944,51,107,'','
',1,10,'',0
--select * from Seguridad.Usuario where IdUsuario=24 and Logueo like '%uaf137%'
ALTER PROCEDURE [Tramite].[paListarDetalleBusquedaExpedienteGeneral]
       @pIdExpediente int,
	   @pIdArea int,
       @pIdUsuarioAuditoria int,
       @pCampoOrdenado varchar(50),
...
AS
	   ...
	   SET LANGUAGE 'SPANISH'
			...
			DECLARE @vIdTipoFormulario INT=0
			SELECT @vIdTipoFormulario=COUNT(IdTipoFormulario) FROM Tramite.PermisoVisualizacionDocumentos WHERE IdPersona=@vIdPersonaActual AND IdTipoFormulario=3 and convert(date, GETDATE() )between convert(date,FechaInicioPersmiso) and convert(date,FechaFinPersmiso)
			...
			-------------------------------------------------------------------------------------------------------------------------------------
             ...
             SET @Orden=' ORDER BY CONVERT(DATETIME,EDO.FechaOrigen+'' ''+EDO.HoraOrigen)
  DESC '
             SET @Offset= ' OFFSET ' +CONVERT(VARCHAR(10),(@pNumeroPagina-1)*@pDimensionPagina) + ' ROWS'
             SET @Fetch= ' FETCH NEXT '+CONVERT(VARCHAR(10),@pDimensionPagina) +' ROWS ONLY'
