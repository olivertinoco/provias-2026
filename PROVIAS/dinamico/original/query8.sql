CREATE PROCEDURE [Tramite].[paListarExpedientePendienteJefaturaTodos]
 @pConFiltroFecha bit,
...
AS
 BEGIN TRY
  ...
  SELECT IdCargo FROM General.Cargo WHERE IdCatalogoTipoCargo in (32,33,34)
   ...
   --IF ISNUMERIC(@pBusquedaGeneral)=0 AND @pBusquedaGeneral IS NOT NULL AND  @pBusquedaGeneral<>''
   --BEGIN
