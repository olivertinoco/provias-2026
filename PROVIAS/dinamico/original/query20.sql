CREATE  PROCEDURE [Tramite].[paListarTreeExpedienteDocumentoOrigen]
       @pIdExpediente int,
...
BEGIN TRY
        ...
        where o.EstadoAuditoria=1 and ed.IdExpediente=@pIdExpediente and o.EsCabecera = 1 --and o.EsInicial=1
            AND ed.EstadoAuditoria=1 AND e.EstadoAuditoria=1


END TRY
BEGIN CATCH
    DECLARE @ERROR_NUMBER INT, @ERROR_SEVERITY INT,@ERROR_STATE INT,@ERROR_LINE INT,@ERROR_PROCEDURE VARCHAR(MAX) ,@ERROR_MESSAGE VARCHAR(MAX)
    SELECT @ERROR_NUMBER=ERROR_NUMBER() , @ERROR_SEVERITY=ERROR_SEVERITY() , @ERROR_STATE=ERROR_STATE() , @ERROR_PROCEDURE='Tramite.paListarTreeExpedienteDocumentoOrigen',@ERROR_LINE=ERROR_LINE(),@ERROR_MESSAGE=ERROR_MESSAGE()
