CREATE PROCEDURE [Tramite].[paListarExpedienteBusquedaPendiente]
...
AS
	BEGIN TRY
	--	declare @pIdUsuarioAuditoria int=1109,
	...
	--@pIdPeriodo INT=2024,
	--@pIdSerieDocumental INT=0,
 --   @pNombreProyecto varchar(1000)='',
 --   @pNumeroExpediente  varchar(1000)='',
 --   @pFechaExpediente  varchar(1000)='',
 --   @pAsuntoExpediente  varchar(1000)='',
 --   @pIdCatategoriaExpediente INT=0,
 --   @pIdPrioridadExpediente INT=0,
 --   @pNumeroDocumento varchar(50)='',
 --   @pFechaDocumento varchar(10)='',
 ...
 --   @pAsuntoDocumento varchar(1000)='',
	...
	--@pIdCargoOrigen INT=0,
	--@pIdCargoDestino INT=0,
	--@pEmisor varchar(1000)='claro',
 --   @pDestinatario  varchar(1000)=null

		if (@pIdPeriodo = 0)
			select @pIdPeriodo=YEAR(GETDATE())
		...
		--EXTRAEREMOS EL AREA DEL USUARIO QUE CONSULTARA (SOLO PODRA CONSULTAR EXPEDIENTES DONDE EL ESTA PRESENTE)
		DECLARE @vIdPersonaU int
		SELECT @vIdPersonaU=IdPersona FROM Seguridad.Usuario WHERE IdUsuario= @pIdUsuarioAuditoria

		IF COALESCE(@pIdPeriodo,0)<>0 BEGIN	SET @FiltroExpediente =@FiltroExpediente+ ' AND E.IdPeriodo ='+CONVERT(VARCHAR,@pIdPeriodo) END
		IF COALESCE(@pIdSerieDocumental,0)<>0 BEGIN	SET @FiltroExpediente =@FiltroExpediente+ ' AND E.IdSerieDocumentalExpediente ='+CONVERT(VARCHAR,@pIdSerieDocumental) END
		IF COALESCE(@pIdCatategoriaExpediente,0)<>0 BEGIN	SET @FiltroExpediente =@FiltroExpediente+ ' AND E.IdCatalogoTipoTramite ='+CONVERT(VARCHAR,@pIdCatategoriaExpediente) END
		IF COALESCE(@pIdPrioridadExpediente,0)<>0 BEGIN	SET @FiltroExpediente =@FiltroExpediente+ ' AND E.IdCatalogoTipoPrioridad ='+CONVERT(VARCHAR,@pIdPrioridadExpediente) END
		IF COALESCE(@pIdCatalogoTipoDocumento,0)<>0 BEGIN	SET @FiltroExpediente =@FiltroExpediente+ ' AND ED.IdCatalogoTipoDocumento ='+CONVERT(VARCHAR,@pIdCatalogoTipoDocumento) END
		IF COALESCE(@pAsuntoExpediente,'')<>'' BEGIN	SET @FiltroExpediente =@FiltroExpediente+' AND E.AsuntoExpediente LIKE ''%'+@pAsuntoExpediente +'%'''END

		IF COALESCE(@pAsuntoDocumento,'')<>'' BEGIN	SET @FiltroExpediente =@FiltroExpediente+' AND ED.AsuntoDocumento LIKE ''%'+@pAsuntoDocumento +'%'''END
		IF COALESCE(@pFechaDocumento,'')<>'' BEGIN	SET @FiltroExpediente =@FiltroExpediente+' AND ED.NFechaDocumento LIKE ''%'+@pFechaDocumento+'%''' END
		IF COALESCE(@pNumeroDocumento,'')<>'' BEGIN	SET @FiltroExpediente =@FiltroExpediente+' AND ED.NumeroDocumento LIKE ''%'+@pNumeroDocumento +'%'''END
		IF COALESCE(@pNumeroExpediente,'')<>'' BEGIN SET @FiltroExpediente =@FiltroExpediente+' AND SD.AbreviaturaSerieDocumentalExpediente +RIGHT(''000000''+CONVERT(VARCHAR,E.NumeroExpediente),6) LIKE ''%' +@pNumeroExpediente+'%''' END
		--IF COALESCE(@pNombreProyecto,'')<>'' BEGIN	SET @FiltroExpediente =@FiltroExpediente+' AND P.NombreProyecto LIKE ''%'+@pNombreProyecto +'%''' END

		IF COALESCE(@pEmisor,'')<>'' BEGIN	SET @FiltroExpediente =@FiltroExpediente+' AND (PD.NombreCompleto LIKE ''%'+@pEmisor +'%'' or  E.NombreCompletoCreador LIKE ''%'+@pEmisor +'%'')' END
		...
		begin
			IF COALESCE(@pBusquedaGeneral,'')<>'' SET @Filtros ='AND (CONCAT(SD.AbreviaturaSerieDocumentalExpediente +RIGHT(''000000''+CONVERT(VARCHAR,E.NumeroExpediente),6), ''-'', E.IdPeriodo) LIKE ''%'+@pBusquedaGeneral +'%''  OR E.AsuntoExpediente LIKE ''%'+@pBusquedaGeneral +'%'' or PD.NombreCompleto LIKE ''%'+@pBusquedaGeneral +'%'' or  E.NombreCompletoCreador LIKE ''%'+@pBusquedaGeneral +'%'')'
		end

		--VERIFICAR SI TIENE PERMISO PARA VER DOCUMENTOS DE TODAS LAS AREAS 4
		DECLARE @vIdTipoFormulario INT=0
		DECLARE @Filtrospermisos VARCHAR(MAX)
		SELECT @vIdTipoFormulario=COUNT(IdTipoFormulario) FROM Tramite.PermisoVisualizacionDocumentos WHERE IdPersona=@vIdPersonaU AND IdTipoFormulario=4 and convert(date, GETDATE() )between convert(date,FechaInicioPersmiso) and convert(date,FechaFinPersmiso)
		SET @vIdTipoFormulario=4--LUEGO VALIDAR ESTO
		...
		ELSE
		BEGIN
			--LA PRIMERA BUSQUEDA NO TRAERA NINGUN REGISTRO
			IF @FiltroExpediente=''
			BEGIN
		 		IF @Filtros=''
				BEGIN
				...
				END
			END
			--SET @Filtrospermisos =' WHERE  E.EstadoAuditoria=1 AND (EDO.IdAreaOrigen IN (SELECT IdArea FROM RecursoHumano.visEmpleadoPerfilPersona WHERE IdPersona='+LTRIM(@vIdPersonaU)+' AND Activo=1)  or EDOD.IdAreaDestino IN (SELECT IdArea FROM RecursoHumano.visEmpleadoPerfilPersona WHERE IdPersona='+LTRIM(@vIdPersonaU)+' AND Activo=1)) AND EDOD.EstadoAuditoria=1 '+@FiltroExpediente+') '
			SET @Filtrospermisos =' WHERE  E.EstadoAuditoria=1 AND (EDO.IdAreaOrigen IN (SELECT IdArea FROM RecursoHumano.visEmpleadoPerfilPersona WHERE IdPersona='+LTRIM(@vIdPersonaU)+' AND Activo=1)  or EDOD.IdAreaDestino IN (SELECT IdArea FROM RecursoHumano.visEmpleadoPerfilPersona WHERE IdPersona='+LTRIM(@vIdPersonaU)+' AND Activo=1)) AND EDOD.EstadoAuditoria=1 '+@FiltroExpediente+' '
		END

		IF COALESCE(@pNumeroDocumento,'')<>''
		BEGIN
			SET @Documentos = ' select max(ED.NumeroDocumento) documento from Tramite.ExpedienteDocumento ED where ED.IdExpediente=E.IdExpediente AND ED.NumeroDocumento LIKE ''%'+@pNumeroDocumento +'%'''
		END
		ELSE
		BEGIN
		...
		--	WHERE E.EstadoAuditoria=1 AND E.ExpedienteAnulado=0  AND E.IdExpediente IN (
		--	SELECT  E.IdExpediente FROM
		--	Tramite.Expediente E
		--	INNER JOIN Tramite.SerieDocumentalExpediente SD ON SD.IdSerieDocumentalExpediente=E.IdSerieDocumentalExpediente
		--	INNER JOIN Tramite.ExpedienteDocumento ED ON E.IdExpediente=ED.IdExpediente
		...
		--	LEFT JOIN General.Persona PD ON E.IdPersonaCreador=PD.IdPersona
		--	WHERE E.EstadoAuditoria=1 AND E.ExpedienteAnulado=0  AND E.IdExpediente IN (
		--	SELECT top 500 E.IdExpediente FROM
		--	Tramite.Expediente E
		--	INNER JOIN Tramite.SerieDocumentalExpediente SD ON SD.IdSerieDocumentalExpediente=E.IdSerieDocumentalExpediente
		--	INNER JOIN Tramite.ExpedienteDocumento ED ON E.IdExpediente=ED.IdExpediente
		--	INNER JOIN Tramite.ExpedienteDocumentoOrigen EDO ON EDO.IdExpedienteDocumento=ED.IdExpedienteDocumento AND ED.EstadoAuditoria=1
	 ...
	 --          order by E.IdExpediente desc
		...
		SET @ConsultaTotal = '
			...
			DECLARE @vTablaExpediente TABLE(IdExpediente int)
		    INSERT INTO @vTablaExpediente
			SELECT top 100 E.IdExpediente FROM
			Tramite.Expediente E
			INNER JOIN Tramite.SerieDocumentalExpediente SD ON SD.IdSerieDocumentalExpediente=E.IdSerieDocumentalExpediente
			INNER JOIN Tramite.ExpedienteDocumento ED ON E.IdExpediente=ED.IdExpediente and E.EstadoAuditoria=1 and E.ExpedienteAnulado=0
			INNER JOIN Tramite.ExpedienteDocumentoOrigen EDO ON EDO.IdExpedienteDocumento=ED.IdExpedienteDocumento AND ED.EstadoAuditoria=1
			INNER JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD ON EDOD.IdExpedienteDocumentoOrigen=EDO.IdExpedienteDocumentoOrigen  AND EDO.EstadoAuditoria=1
			LEFT JOIN General.Persona PD ON E.IdPersonaCreador=PD.IdPersona '
			+ @Filtrospermisos
