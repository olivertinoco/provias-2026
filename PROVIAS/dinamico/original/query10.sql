	...
	ELSE
    	BEGIN
    		...
    		BEGIN
    			...
    			END
    		END
    		SET @Filtrospermisos =' WHERE  E.EstadoAuditoria=1 AND (EDO.IdAreaOrigen IN (SELECT IdArea FROM RecursoHumano.visEmpleadoPerfilPersona WHERE IdPersona='+LTRIM(@vIdPersonaU)+' AND Activo=1)  or EDOD.IdAreaDestino IN (SELECT IdArea FROM RecursoHumano.visEmpleadoPerfilPersona WHERE IdPersona='+
