...
SET NOCOUNT ON
	...
	IF @pIdUsuarioAuditoria in(select IdUsuario from Tramite.PermisoVisualizacionDocumentos
	where IdTipoFormulario=2 and EstadoAuditoria=1 and convert(date, GETDATE() )between convert(date,FechaInicioPersmiso) and convert(date,FechaFinPersmiso))
    	BEGIN
    		SET @vSiPariticipo=1
    	END
	ELSE
    	BEGIN
    		...
    		WHERE ED.IdExpediente=@pIdExpediente AND ED.IdPeriodo = @pIdPeriodo AND ( EDOD.IdPersonaDestino = @vIdPersonaActual OR EDO.IdPersonaOrigen=@vIdPersonaActual))
    	END
    ...
    SELECT
		...
		ELSE CASE WHEN EE.IdCatalogoSituacionEnvio = 14 THEN EDOD.IdCatalogoSituacionMovimientoDestino ELSE EE.IdCatalogoSituacionEnvio END END IdCatalogoSituacionMovimientoDestino,
        CASE WHEN EE.IdEnvio IS NULL THEN CSM.Descripcion ELSE CASE WHEN EE.IdCatalogoSituacionEnvio = 14 THEN CSM.Descripcion ELSE CSMEE.Descripcion END END CatalogoSituacionMovimientoDestino,
		...
		CASE WHEN EDO.IdPersonaOrigen=0 THEN coalesce(EDO.NombreCompletoOrigen,'') ELSE CASE WHEN CTM.IdCatalogo=71 THEN coalesce(EDO.NombreCompletoOrigen,'')  ELSE coalesce(PO.NombreCompleto,'') END END  NombrePersonaOrigen,
        ...
        ED.IdExpedienteDocumento,
        CASE WHEN @vEsJefe=1 THEN 0 ELSE Tramite.funEsPropioEspecialista(EDOD.IdExpedienteDocumentoOrigenDestino,@pIdEmpresa,@pIdArea,@pIdCargo,@pIdPersona) END EsPropio,
		CASE WHEN @vEsJefe=1 THEN 0 ELSE CASE WHEN ED.IdCargoEmisor=@pIdCargo and ED.IdAreaEmisor=@pIdArea and ED.IdEmpresaEmisor=@pIdEmpresa and ED.IdPersonaEmisor=@pIdPersona THEN 1 ELSE 0 END END EsMiDocumento,
		CASE WHEN @vEsJefe=1 THEN 0 ELSE CASE WHEN EDO.IdCargoOrigen=@pIdCargo and EDO.IdAreaOrigen=@pIdArea and EDO.IdEmpresaOrigen=@pIdEmpresa and EDO.IdPersonaOrigen=@pIdPersona THEN 1 ELSE 0 END END EsOrigen,
        ED.IdExpedienteDocumento,
        ...
        INNER JOIN Tramite.ExpedienteDocumento ED WITH(NOLOCK) ON ED.IdExpediente=E.IdExpediente AND ED.IdPeriodo = @pIdPeriodo
        INNER JOIN Tramite.ExpedienteDocumentoOrigen EDO WITH(NOLOCK) ON EDO.IdExpedienteDocumento=ED.IdExpedienteDocumento AND ED.EstadoAuditoria=1 AND EDO.IdPeriodo = @pIdPeriodo
        INNER JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD WITH(NOLOCK) ON EDOD.IdExpedienteDocumentoOrigen=EDO.IdExpedienteDocumentoOrigen  AND EDO.EstadoAuditoria=1 AND EDOD.EstadoAuditoria=1 AND EDOD.IdPeriodo = @pIdPeriodo
		...
		outer apply(
			...
			where EDF.IdExpedienteDocumento=ED.IdExpedienteDocumento and EDF.IdPersona=@vIdPersonaActual and EDF.EstadoAuditoria=1 AND EDF.IdPeriodo = @pIdPeriodo
		) Ver
...
BEGIN CATCH
	...
	@ERROR_PROCEDURE='Tramite.paListarDocumentoPendienteEspecialistaV1',@ERROR_LINE=ERROR_LINE(),@ERROR_MESSAGE=ERROR_MESSAGE()
