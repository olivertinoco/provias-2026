ALTER PROCEDURE [Tramite].[paListarExpedientePendienteCourrierJefatura]
	@pIdArea int,
...
SET LANGUAGE SPANISH
	...
	BEGIN
		...
		WHERE E.EstadoAuditoria=1 AND E.IdPeriodo = @pIdPeriodo)X WHERE 1=1 ' + @Filtros
		...
		INNER JOIN Tramite.ExpedienteDocumentoOrigen EDO ON EDO.IdExpedienteDocumento=ED.IdExpedienteDocumento AND ED.EstadoAuditoria=1 AND ED.IdAreaEmisor='+CONVERT(VARCHAR,@vIdAreaJefe)+
		' AND ED.IdCargoEmisor in(SELECT IdCargo FROM General.Cargo WHERE IdCatalogoTipoCargo in (32,33,34)) AND EDO.IdPeriodo = @pIdPeriodo
		INNER JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD ON EDOD.IdExpedienteDocumentoOrigen=EDO.IdExpedienteDocumentoOrigen AND EDO.EstadoAuditoria=1 AND EDOD.IdCatalogoTipoMovimientoDestino=72 AND EDOD.IdPeriodo = @pIdPeriodo
		WHERE  EDOD.EstadoAuditoria=1 AND ED.IdPeriodo = @pIdPeriodo
		...
		LEFT JOIN General.Persona PE ON PE.IdPersona=E.IdPersonaCreador
		LEFT JOIN Tramite.Catalogo CTT ON CTT.IdCatalogo=E.IdCatalogoTipoTramite
		WHERE E.EstadoAuditoria=1 AND E.IdPeriodo = @pIdPeriodo)X WHERE 1=1 ' +@Filtros + @Orden + @Offset + @Fetch

		set @Consulta2 = @Consulta + ' ' + @ConsultaTotal

		EXECUTE sp_executesql @Consulta2, N'@pIdPeriodo int', @pIdPeriodo
	...
	BEGIN
		...
		SET @ConsultaTotal = N'SELECT count(*)FROM (SELECT
		Tramite.funParaAnularJefatura(E.IdExpediente,'+CONVERT(VARCHAR,@pIdArea)+','+CONVERT(VARCHAR,@vIdCargoJefe)+') EsParaAnular,
	    E.ExpedienteAnulado,
	    COALESCE(E.MotivoExpedienteAnulado,'''')MotivoExpedienteAnulado,
	    COALESCE(E.NFechaAnulacionExpediente,'''')NFechaAnulacionExpediente,
		...
		LEFT JOIN General.Persona PE ON PE.IdPersona=E.IdPersonaCreador
		LEFT JOIN Tramite.Catalogo CTT ON CTT.IdCatalogo=E.IdCatalogoTipoTramite
		WHERE E.EstadoAuditoria=1 AND E.IdPeriodo = @pIdPeriodo) X  WHERE X.EsMiAnulado=X.ExpedienteAnulado ' + @Filtros
		...
		INNER JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD ON EDOD.IdExpedienteDocumentoOrigen=EDO.IdExpedienteDocumentoOrigen  AND EDO.EstadoAuditoria=1 AND EDOD.EstadoAuditoria=1 AND EDOD.IdCatalogoTipoMovimientoDestino=72 AND EDOD.IdPeriodo = @pIdPeriodo
		...
		WHERE ED.IdPeriodo = @pIdPeriodo ' +@vTipoPendiente
		+' SELECT * FROM (SELECT
		Tramite.funParaAnularJefatura(E.IdExpediente,'+CONVERT(VARCHAR,@pIdArea)+','+CONVERT(VARCHAR,@vIdCargoJefe)+') EsParaAnular,
	    E.ExpedienteAnulado,
	    COALESCE(E.MotivoExpedienteAnulado,'''')MotivoExpedienteAnulado,
		...
		CONVERT(DATETIME,E.NTFechaExpediente +'' ''+ E.HoraExpediente) Fecha,
		Tramite.funEsMiAnuladoJefatura(E.IdExpediente,'+CONVERT(VARCHAR,@pIdArea)+','+CONVERT(VARCHAR,@vIdCargoJefe)+') EsMiAnulado,
		CONCAT(SD.AbreviaturaSerieDocumentalExpediente, RIGHT(100000+E.NumeroExpediente,5), ''-'', E.IdPeriodo) NombreExpediente,
		CASE WHEN COALESCE(E.IdPersonaCreador,0)=0 THEN COALESCE(E.NombreCompletoCreador,'''') ELSE PE.NombreCompleto END NombreCompletoCreador
		FROM Tramite.Expediente E
		INNER JOIN @vTablaExpediente EE ON EE.IdExpediente=E.IdExpediente
		INNER JOIN Seguridad.Usuario US ON US.IdUsuario=E.IdUsuarioCreacionAuditoria
		...
		LEFT JOIN General.Persona PE ON PE.IdPersona=E.IdPersonaCreador
		LEFT JOIN Tramite.Catalogo CTT ON CTT.IdCatalogo=E.IdCatalogoTipoTramite
		WHERE E.EstadoAuditoria=1 AND E.IdPeriodo = @pIdPeriodo) X  WHERE X.EsMiAnulado=X.ExpedienteAnulado ' + @Filtros + @Orden + @Offset + @Fetch
