ALTER PROCEDURE [Tramite].[paListarMisDocumentosGeneradosEspecialistaV1]
    @pIdPersona int,
...
set language spanish
	...
	IF COALESCE(@pIdPeriodo,0)<>0 BEGIN	SET  @vFiltroDocumento=' AND X.IdPeriodo ='+CONVERT(VARCHAR,@pIdPeriodo) END
	...
	IF COALESCE(@pFechaDocumento,'')<>'' BEGIN	SET @vFiltroDocumento =@vFiltroDocumento+' AND convert(date,X.NFechaDocumento) between '''+@vFechaInicial+''' AND '''+@vFechaFinal+'''' END
    ...
    SET @Filtros =' AND (X.NombreExpediente LIKE ''%'+@pBusquedaGeneral +'%'' or X.AsuntoDocumento LIKE ''%'+
    @pBusquedaGeneral +'%'' or X.NumeroDocumento LIKE ''%'+@pBusquedaGeneral +'%'' or X.ObservacionesDocumento LIKE ''%'+@pBusquedaGeneral +'%'' or X.NFechaDocumento LIKE ''%'+@pBusquedaGeneral +'%'')'
	...
	INNER JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD ON EDOD.IdExpedienteDocumentoOrigen=EDO.IdExpedienteDocumentoOrigen AND EDO.EstadoAuditoria=1 and edod.EsInicial<>0 AND EDOD.IdPeriodo = @pIdPeriodo
	INNER JOIN Tramite.Catalogo CTD ON CTD.IdCatalogo=ED.IdCatalogoTipoDocumento
	LEFT JOIN Tramite.ExpedienteSeguimiento ES ON ES.IdExpediente= E.IdExpediente AND ES.EstadoAuditoria=1 AND ES.IdEmpresa=2 AND ES.IdPersona='+CONVERT(VARCHAR,@pIdPersona)+' AND ES.IdPeriodo = @pIdPeriodo
	...
	WHERE EDOD.EstadoAuditoria=1 AND ED.IdPersonaEmisor=' + CONVERT(VARCHAR,@pIdPersona) + ' AND E.IdPeriodo = @pIdPeriodo)X WHERE 1=1 ' + @vFiltroDocumento + @Filtros
	...
	LEFT JOIN General.Area A ON A.IdArea=ED.IdAreaEmisor
	LEFT JOIN General.Empresa EM ON EM.IdEmpresa=ED.IdEmpresaEmisor
	WHERE EDOD.EstadoAuditoria=1 AND ED.IdPersonaEmisor='+CONVERT(VARCHAR,@pIdPersona)+' AND E.IdPeriodo = @pIdPeriodo)X WHERE 1=1 '+ @vFiltroDocumento + @Filtros + @Orden + @Offset + @Fetch
...
BEGIN CATCH
    ...
    @ERROR_PROCEDURE='Tramite.paListarMisDocumentosGeneradosEspecialistaV1',@ERROR_LINE=ERROR_LINE(),@ERROR_MESSAGE=ERROR_MESSAGE()
