...
BEGIN TRY
    ...
    e.NumeroExpediente,
	CONCAT(SD.AbreviaturaSerieDocumentalExpediente, RIGHT(1000000+E.NumeroExpediente,6), '-', E.IdPeriodo, CASE WHEN COALESCE(ED.CorrelativoVinculado,0)=0 THEN '' ELSE '-'+LTRIM(ED.CorrelativoVinculado) END) NombreExpediente,
    e.IdPeriodo,
    ...
    INNER JOIN Tramite.ExpedienteDocumento ed on ed.IdExpedienteDocumento=o.IdExpedienteDocumento AND ED.IdPeriodo = @pIdPeriodo
    INNER JOIN Tramite.Expediente e on e.IdExpediente=ed.IdExpediente AND E.IdPeriodo = @pIdPeriodo
    ...
    where o.EstadoAuditoria=1 and ed.IdExpediente=@pIdExpediente and o.EsCabecera = 1 AND ed.EstadoAuditoria=1 AND e.EstadoAuditoria=1 AND o.IdPeriodo = @pIdPeriodo
