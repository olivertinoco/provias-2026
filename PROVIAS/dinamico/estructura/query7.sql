ALTER PROCEDURE [Tramite].[paListarDocumentoOrigenDestinoHojaRuta]
    @pIdExpediente int,
    @pIdArea int,
    @pEsVinculado int,
...
SET LANGUAGE SPANISH
	...
	CONCAT(SD.AbreviaturaSerieDocumentalExpediente,RIGHT(1000000+E.NumeroExpediente,6), '-', E.IdPeriodo,CASE WHEN ED.EsVinculado=1 THEN CONCAT('- V-',ED.CorrelativoVinculado) else '' END)NombreExpediente,
    ...
    INNER JOIN Tramite.ExpedienteDocumento ED ON ED.IdExpediente=E.IdExpediente and ED.EstadoAuditoria=1 AND ED.IdPeriodo = @pIdPeriodo
    INNER JOIN Tramite.ExpedienteDocumentoOrigen EDO ON EDO.IdExpedienteDocumento=ED.IdExpedienteDocumento AND EDO.EstadoAuditoria=1 AND EDO.IdPeriodo = @pIdPeriodo
    INNER JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD ON EDOD.IdExpedienteDocumentoOrigen=EDO.IdExpedienteDocumentoOrigen  AND EDOD.EstadoAuditoria=1 AND EDOD.IdPeriodo = @pIdPeriodo
    ...
    WHERE E.EstadoAuditoria=1 AND E.IdExpediente=@pIdExpediente and coalesce(ED.CorrelativoVinculado,0)=@pCorrelativoVinculado AND E.IdPeriodo = @pIdPeriodo
...
BEGIN CATCH
    ...
    @ERROR_PROCEDURE='Tramite.paListarDocumentoOrigenDestinoHojaRuta',@ERROR_LINE=ERROR_LINE(),@ERROR_MESSAGE=ERROR_MESSAGE()
