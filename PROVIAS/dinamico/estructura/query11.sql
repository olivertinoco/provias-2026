ALTER PROCEDURE [Tramite].[paListarMisDocumentosGeneradosJefatura]
    @pIdAreaEmisor int,
    @pIdPersona int,
...
set language spanish
    ...
    SET @Filtros =' AND (X.NombreExpediente LIKE ''%'+@pBusquedaGeneral +'%'' or X.AsuntoDocumento LIKE ''%'+@pBusquedaGeneral +'%'' or X.NumeroDocumento LIKE ''%'+
    @pBusquedaGeneral +'%'' or X.ObservacionesDocumento LIKE ''%'+@pBusquedaGeneral +'%'' or X.NFechaDocumento LIKE ''%'+@pBusquedaGeneral +'%''  or X.Destinatario LIKE ''%'+@pBusquedaGeneral +'%'')'
	...
	E.IdExpediente,
	CONCAT(SD.AbreviaturaSerieDocumentalExpediente, RIGHT(1000000+E.NumeroExpediente,6), ''-'', E.IdPeriodo,CASE WHEN COALESCE(ED.CorrelativoVinculado,0)=0 THEN '''' ELSE '' V-''+CONVERT(VARCHAR,ED.CorrelativoVinculado) END) NombreExpediente,
	E.ExpedienteAnulado,
	...
	INNER JOIN Tramite.ExpedienteDocumento ED with (nolock) ON ED.IdExpediente=E.IdExpediente and e.EstadoAuditoria=1 AND ED.IdPeriodo = @pIdPeriodo
	...
	INNER JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD  with (nolock) ON EDOD.IdExpedienteDocumentoOrigen=EDO.IdExpedienteDocumentoOrigen  AND EDO.EstadoAuditoria=1  and edod.EsInicial<>0 AND EDOD.IdPeriodo = @pIdPeriodo
	INNER JOIN Tramite.Catalogo CTD ON CTD.IdCatalogo=ED.IdCatalogoTipoDocumento
	LEFT JOIN Tramite.ExpedienteSeguimiento ES ON ES.IdExpediente= E.IdExpediente AND ES.EstadoAuditoria=1 AND ES.IdCargo=0 AND ES.IdPersona=0 AND ES.IdArea='+CONVERT(VARCHAR,@pIdAreaEmisor)+' AND ES.IdPeriodo = @pIdPeriodo
	...
	WHERE E.IdPeriodo = @pIdPeriodo AND EDOD.EstadoAuditoria=1 and ED.IdAreaEmisor='+CONVERT(VARCHAR,@pIdAreaEmisor)+' '+@vFiltroJefe+'
	)X WHERE 1=1 '+@vFiltroDocumento + @Filtros
	...
	LEFT JOIN General.Persona P ON P.IdPersona=ED.IdPersonaEmisor
	WHERE E.IdPeriodo = @pIdPeriodo AND EDOD.EstadoAuditoria=1 and ED.IdAreaEmisor='+CONVERT(VARCHAR,@pIdAreaEmisor)+' '+@vFiltroJefe+'
	)X WHERE 1=1 '+ @vFiltroDocumento + @Filtros + @Orden + @Offset + @Fetch
