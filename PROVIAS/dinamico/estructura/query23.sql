...
SET NOCOUNT ON;
	...
	IF COALESCE(@pBusquedaGeneral,'')<>'' SET @Filtros ='AND (CONCAT(SD.AbreviaturaSerieDocumentalExpediente, RIGHT(1000000+E.NumeroExpediente,6), ''-'', E.IdPeriodo) LIKE ''%'+@pBusquedaGeneral +'%'')'
	...
	INNER JOIN Tramite.ExpedienteDocumento ED ON ED.IdExpediente=E.IdExpediente  and e.EstadoAuditoria=1 AND ED.EstadoAuditoria=1 AND ED.FgEnEsperaFirmaDigital=1 AND ED.IdPeriodo = @pIdPeriodo
	...
	+' AND EDF.IdPeriodo = @pIdPeriodo  WHERE E.IdPeriodo = @pIdPeriodo AND EDF.FlagFirmado=0 AND FgEsObligatorioFirmaDigital=1 ' + @Filtros
	...
	(SELECT COUNT(IdExpedienteDocumento) FROM Tramite.ExpedienteDocumento WHERE IdUsuarioEnProcesoFirma ='+CONVERT(varchar,@pIdUsuarioAuditoria)+' AND EnProcesoFirma=1 AND IdExpedienteDocumento=ED.IdExpedienteDocumento AND EstadoAuditoria=1 AND IdPeriodo = @pIdPeriodo) EsLiberado,
	...
	WHERE E.IdPeriodo = @pIdPeriodo AND EDF.FlagFirmado=0 AND FgEsObligatorioFirmaDigital=1 ' + @Filtros + @Orden + @Offset + @Fetch
