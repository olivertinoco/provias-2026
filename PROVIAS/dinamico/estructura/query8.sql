...
BEGIN TRY
   ...
   LEFT JOIN Tramite.ExpedienteSeguimiento ES WITH (NOLOCK) ON ES.IdExpediente= E.IdExpediente AND ES.EstadoAuditoria=1 AND ES.IdCargo=0 AND ES.IdPersona=0 AND ES.IdArea=@pIdArea AND ES.IdPeriodo = @pIdPeriodo
   ...
   LEFT JOIN General.Persona PE ON PE.IdPersona=E.IdPersonaCreador
   LEFT JOIN Tramite.Catalogo CTT ON CTT.IdCatalogo=E.IdCatalogoTipoTramite
   WHERE (E.NumeroExpediente =  @pBusquedaGeneral OR @pBusquedaGeneral IS NULL OR @pBusquedaGeneral=0) AND E.IdPeriodo = @pIdPeriodo
