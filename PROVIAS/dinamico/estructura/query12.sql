...
SET LANGUAGE SPANISH
	...
	SELECT @vIdTipoFormulario=COUNT(IdTipoFormulario) FROM Tramite.PermisoVisualizacionDocumentos
	WHERE EstadoAuditoria=1 and IdPersona=@vIdPersonaActual AND IdTipoFormulario=3 and convert(date, GETDATE() )between convert(date,FechaInicioPersmiso) and convert(date,FechaFinPersmiso)
	...
	ELSE
		BEGIN
    		...
    		BEGIN
    			...
    			INNER JOIN Tramite.ExpedienteDocumentoOrigen EDO ON EDO.IdExpedienteDocumento=ED.IdExpedienteDocumento AND ED.EstadoAuditoria=1 AND EDO.IdPeriodo = @pIdPeriodo
    			INNER JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD ON EDOD.IdExpedienteDocumentoOrigen=EDO.IdExpedienteDocumentoOrigen  AND EDO.EstadoAuditoria=1 AND EDOD.IdPeriodo = @pIdPeriodo
    			WHERE ED.IdPeriodo = @pIdPeriodo AND ED.IdExpediente=@pIdExpediente AND ( EDOD.IdPersonaDestino = @vIdPersonaActual OR EDO.IdPersonaOrigen=@vIdPersonaActual))
    		END
	    END
    ...
    WHERE E.IdPeriodo = @pIdPeriodo AND EDOD.EstadoAuditoria=1 AND E.IdExpediente=' + CONVERT(VARCHAR,@pIdExpediente) + @Filtros + @vCondicionVinculado
	...
	outer apply(
		...
		where EDF.IdExpedienteDocumento=ED.IdExpedienteDocumento and EDF.IdPersona= ' + convert(varchar,@vIdPersonaActual) + ' and EDF.EstadoAuditoria=1 AND EDF.IdPeriodo = @pIdPeriodo
	) Ver
    LEFT JOIN General.Empresa EMO ON EMO.IdEmpresa=EDO.IdEmpresaOrigen
    LEFT JOIN General.Area AO ON AO.IdArea= EDO.IdAreaOrigen
    LEFT JOIN General.Cargo CO ON CO.IdCargo=EDO.IdCargoOrigen
    ...
    LEFT JOIN General.Persona PA ON PA.IdPersona=EDOD.IdPersonaDestinoAtencion
	LEFT JOIN Courrier.Envio EE ON EE.IdExpedienteDocumentoOrigenDestino =	EDOD.IdExpedienteDocumentoOrigenDestino AND EE.EstadoAuditoria=1 AND FgEntregado=0
    WHERE E.IdPeriodo = @pIdPeriodo AND EDOD.EstadoAuditoria=1 AND E.IdExpediente=' + CONVERT(VARCHAR,@pIdExpediente) + @Filtros + @vCondicionVinculado + @Orden + @Offset + @Fetch
