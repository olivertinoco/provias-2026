ALTER PROCEDURE [Tramite].[paListarExpedienteAcervoDocumentalExportarExcelJefaturaV1]
    @pIdAreaEmisor INT,
    @pIdPersona INT,
...
set language spanish
    ...
    SELECT Periodo,[Tipo Documento],Expediente,Anulado,NombreEmpresa[Razon Social],NombreArea[Nombre Area],NombreCargo[Nombre Cargo],NombreCompleto[Nombre Completo],Documento, [Fecha documento],Asunto,Observaciones,Destinatario,Logueo
	...
	WHERE EDOD.EstadoAuditoria=1 AND (@pIdCatalogoTipoDocumento=0 or ED.IdCatalogoTipoDocumento = @pIdCatalogoTipoDocumento) AND ED.IdAreaEmisor=@pIdAreaEmisor AND (@pIdPersona=0 OR ED.IdPersonaEmisor=@pIdPersona) AND E.IdPeriodo = @pIdPeriodo
	)X WHERE 1=1  AND
...
BEGIN CATCH
    ...
    @ERROR_PROCEDURE='Tramite.paListarExpedienteAcervoDocumentalExportarExcelJefaturaV1',@ERROR_LINE=ERROR_LINE(),@ERROR_MESSAGE=ERROR_MESSAGE()
