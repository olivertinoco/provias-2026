ALTER PROCEDURE [Tramite].[paListarExpedienteBusquedaPendiente]
...
SET LANGUAGE SPANISH
	...
	if (@pIdPeriodo = 0) select @pIdPeriodo=YEAR(GETDATE())
	...
	IF COALESCE(@pNumeroExpediente,'')<>'' BEGIN SET @FiltroExpediente =@FiltroExpediente+' AND SD.AbreviaturaSerieDocumentalExpediente +RIGHT(1000000+E.NumeroExpediente,6) LIKE ''%' +@pNumeroExpediente+'%''' END
	...
	if ISNUMERIC(@pBusquedaGeneral)=1
    	...
    	begin
    		IF COALESCE(@pBusquedaGeneral,'')<>'' SET @Filtros ='AND (CONCAT(SD.AbreviaturaSerieDocumentalExpediente, RIGHT(1000000+E.NumeroExpediente,6), ''-'', E.IdPeriodo) LIKE ''%'+@pBusquedaGeneral +'%''  OR E.AsuntoExpediente LIKE ''%'+@pBusquedaGeneral +'%'' or PD.NombreCompleto LIKE ''%'+@pBusquedaGeneral +'%'' or  E.NombreCompletoCreador LIKE ''%'+@pBusquedaGeneral +'%'')'
    	end
	...
	IF @vIdTipoFormulario>=1
    	BEGIN
    		SET @Filtrospermisos =' WHERE E.IdPeriodo = @pIdPeriodo AND E.EstadoAuditoria=1 AND EDOD.EstadoAuditoria=1 '+@FiltroExpediente+' '
    	END
	ELSE
    	BEGIN
    		IF @FiltroExpediente=''
    		BEGIN
    			...
    			END
    		END
    		SET @Filtrospermisos =' WHERE E.IdPeriodo = @pIdPeriodo AND E.EstadoAuditoria=1 AND (EDO.IdAreaOrigen IN (SELECT IdArea FROM RecursoHumano.visEmpleadoPerfilPersona WHERE IdPersona='+LTRIM(@vIdPersonaU)+' AND Activo=1)  or EDOD.IdAreaDestino IN (SELECT IdArea FROM RecursoHumano.visEmpleadoPerfilPersona WHERE IdPersona='+
            LTRIM(@vIdPersonaU)+' AND Activo=1)) AND EDOD.EstadoAuditoria=1 '+@FiltroExpediente+' '
    	END

	IF COALESCE(@pNumeroDocumento,'')<>''
	BEGIN
		SET @Documentos = ' select max(ED.NumeroDocumento) documento from Tramite.ExpedienteDocumento ED where ED.IdPeriodo = @pIdPeriodo AND ED.IdExpediente=E.IdExpediente AND ED.NumeroDocumento LIKE ''%'+@pNumeroDocumento +'%'''
	END
	ELSE
	BEGIN
	...
	SELECT top 100 E.IdExpediente FROM Tramite.Expediente E
	INNER JOIN Tramite.SerieDocumentalExpediente SD ON SD.IdSerieDocumentalExpediente=E.IdSerieDocumentalExpediente
	INNER JOIN Tramite.ExpedienteDocumento ED ON E.IdExpediente=ED.IdExpediente and E.EstadoAuditoria=1 and E.ExpedienteAnulado=0 AND ED.IdPeriodo = @pIdPeriodo
	...
	+ @Filtrospermisos + @Filtros + ' group by E.IdExpediente order by E.IdExpediente desc '
	...
	+@Documentos
	+' ) ED '
	+ @Orden + @Offset + @Fetch + ' select count(*) from @vTablaExpediente'
...
BEGIN CATCH
	...
	@ERROR_PROCEDURE='Tramite.paListarExpedienteBusquedaPendiente',@ERROR_LINE=ERROR_LINE(),@ERROR_MESSAGE=ERROR_MESSAGE()
