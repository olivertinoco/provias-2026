CREATE PROCEDURE [Tramite].[paListarExpedienteDocumentoHojaRuta]
	@pIdExpediente int,
...
BEGIN TRY
	...
	IF COALESCE(@pBusquedaGeneral,'')<>'' SET @Filtros ='AND (ED.NumeroDocumento LIKE ''%'+@pBusquedaGeneral +'%'' or ED.AsuntoDocumento LIKE ''%'+@pBusquedaGeneral +'%'')'
	SET @ConsultaTotal = N'SELECT @vpTotalRegistros = count(distinct ED.IdExpedienteDocumento)
	...
	INNER JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD ON EDOD.IdExpedienteDocumentoOrigen=EDO.IdExpedienteDocumentoOrigen and EDO.EstadoAuditoria=1 and EDOd.EstadoAuditoria=1
	LEFT JOIN Tramite.Catalogo CTD ON CTD.IdCatalogo=ED.IdCatalogoTipoDocumento
	LEFT JOIN General.Persona p ON p.IdPersona=EDO.IdPersonaOrigen
	LEFT JOIN General.Cargo c ON c.IdCargo=EDO.IdCargoOrigen
	LEFT JOIN General.Area a ON a.IdArea=EDO.IdAreaOrigen
	LEFT JOIN General.Empresa em ON em.IdEmpresa=EDO.IdEmpresaOrigen
	WHERE E.IdExpediente='+LTRIM(@pIdExpediente)+' AND EDOD.IdExpedienteDocumentoOrigenDestinoAnterior=0 AND EDOD.IdExpedienteDocumentoOrigenAnterior=0 AND E.EstadoAuditoria=1 AND ED.EstadoAuditoria=1'
	+@Filtros
	...
	SET @Consulta='
	select distinct E.IdExpediente, ED.IdExpedienteDocumento,COALESCE(EDO.NombreCompletoOrigen,p.NombreCompleto)NombreCompleto,
...
BEGIN CATCH
	...
	SELECT @ERROR_NUMBER=ERROR_NUMBER() , @ERROR_SEVERITY=ERROR_SEVERITY() , @ERROR_STATE=ERROR_STATE() , @ERROR_PROCEDURE='Tramite.paListarExpedienteDocumentoHojaRuta',@ERROR_LINE=ERROR_LINE(),@ERROR_MESSAGE=ERROR_MESSAGE()
