...
--	@pBusquedaGeneral varchar(20)=null
		...
		INNER JOIN Tramite.ExpedienteDocumento ED  WITH (NOLOCK) ON ED.IdExpediente=E.IdExpediente  and e.EstadoAuditoria=1 AND ED.EstadoAuditoria=1 AND ED.FgEnEsperaFirmaDigital=1
...
END CATCH
		...
		--select
		--(SELECT convert(varchar,count(*))
		--FROM Tramite.ExpedienteDocumentoFirmante E
		--WHERE E.EstadoAuditoria=1 and E.IdExpedienteDocumento=1080642 and E.FlagFirmado=0)+'¦'+
		--(select STUFF((
		--SELECT
		--'¬'+COALESCE(Ep.NombreCompleto,'''')
		--FROM Tramite.ExpedienteDocumentoFirmante E
		--INNER JOIN RecursoHumano.visEmpleadoPerfilPersona EP ON EP.IdEmpleadoPerfil=E.IdEmpleadoPerfilFirmante
		--LEFT JOIN Tramite.Catalogo C1 ON C1.IdCatalogo =E.IdCatalogoTipoFirmante
		--LEFT JOIN Tramite.Catalogo C2 ON C2.IdCatalogo =E.IdCatalogoMotivoFirma
		--WHERE E.EstadoAuditoria=1 and E.IdExpedienteDocumento=1080642 and E.FlagFirmado=0
		--FOR XML PATH('')), 1, 1, ''))
