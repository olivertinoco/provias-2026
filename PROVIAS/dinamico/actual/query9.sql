CREATE PROCEDURE [Tramite].[paListarDocumentoPendienteJefatura]
       @pIdExpediente int,
       @pIdArea int,
...
SET LANGUAGE SPANISH;
	...
	------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	 ...
	 IF @pVerSoloMio=1
	 BEGIN
		SET @vCondicionVerSoloMio=' AND CASE WHEN EDOD.IdCargoDestino IN('+@vCargoJefe+') and EDOD.IdAreaDestino='+CONVERT(Nvarchar,@vIdAreaJefe)+' and EDOD.IdEmpresaDestino='+CONVERT(Nvarchar,@vIdEmpresaJefe)+' THEN 1 ELSE 0 END=1'
	 END
	 ...
	 END
        ...
        SET @ConsultaTotal = N'SELECT @vpTotalRegistros = count(*)
            ...
            WHERE EDOD.EstadoAuditoria=1 AND E.IdExpediente='+CONVERT(NVARCHAR,@pIdExpediente)
            +@Filtros
