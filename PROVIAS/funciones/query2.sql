...
BEGIN
	...
	BEGIN
		SELECT
		TOP 1 @vNumeroDocumento='<button type="button" data-toggle="tooltip" title="'+COALESCE(EDOD.MotivoArchivado,'')+
	...
	END
	IF @pIdCatalogoSituacionMovimientoDestino IN(111)
	BEGIN
		SELECT TOP 1 @vNumeroDocumento=
	...
	END
	IF @pIdCatalogoSituacionMovimientoDestino IN(116)
	BEGIN
		SELECT
		TOP 1 @vNumeroDocumento='<button type="button" data-toggle="tooltip" class="btn ui blue label" onclick="MostrarDocumentoPdfExp('''+ED.RutaArchivoDocumento+''','+CONVERT(VARCHAR,ed.IdExpedienteDocumento) +')"><i style="font-size:16px;" class="fa fa-file-text"></i></button><label style="font-size:8px">'+CASE WHEN ED.Correlativo=0 THEN  CONCAT( CTD.Descripcion,' ', COALESCE(ED.NumeroDocumento,'')) ELSE COALESCE(ED.NumeroDocumento,'') END+'</label>'
		FROM
