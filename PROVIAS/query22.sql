	...
	BEGIN TRY
		...
		Begin
			...
			FROM
				...
				INNER JOIN Tramite.ExpedienteDocumento ED WITH (NOLOCK) on ED.IdExpediente=E.IdExpediente AND ED.EstadoAuditoria=E.EstadoAuditoria AND E.IdCatalogoTipoMovimientoTramite=13 AND E.IdCatalogoSituacionExpediente =62 and E.FgTramiteVirtual=1  AND ED.IdExpedienteDocumento Is Not Null and ED.FgDocumentoVirtualEnviado=1
																AND ED.FgEsObservado = 0 and ED.FgEnvioCorregido = 0
				INNER JOIN Tramite.ExpedienteDocumentoOrigen EDO WITH (NOLOCK) on ED.IdExpedienteDocumento=EDO.IdExpedienteDocumento and EDO.EstadoAuditoria=E.EstadoAuditoria
				)
				LEFT JOIN Tramite.Catalogo CTT WITH (NOLOCK) ON CTT.IdCatalogo=E.IdCatalogoTipoTramite
				LEFT JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD WITH (NOLOCK) on EDO.IdExpedienteDocumentoOrigen=EDOD.IdExpedienteDocumentoOrigen and EDOD.EstadoAuditoria=E.EstadoAuditoria
			WHERE E.EstadoAuditoria=1
				...
				--AND (
				--	(CONTAINS(ED.AsuntoDocumento, @pBusquedaGeneralfText))
				--	--(CONTAINS(ED.NumeroDocumento, @pBusquedaGeneralfText))
				--	--(E.NumeroExpediente LIKE '%'+@pBusquedaGeneral +'%') OR
				--	--(CTT.Descripcion LIKE '%'+@pBusquedaGeneral +'%') OR
				--	--(ED.NombreCompletoEmisor LIKE '%'+@pBusquedaGeneral +'%')
				--)
			OPTION (MAXDOP 2)
		End
		Else
		Begin
			...
			FROM
				Tramite.Expediente E WITH (NOLOCK)
				INNER JOIN Tramite.ExpedienteDocumento ED WITH (NOLOCK) ON ED.IdExpediente=E.IdExpediente AND E.IdCatalogoTipoMovimientoTramite=13 AND E.IdCatalogoSituacionExpediente =62 and E.FgTramiteVirtual=1  AND ED.IdExpedienteDocumento Is Not Null and ED.FgDocumentoVirtualEnviado=1
																			AND ED.FgEsObservado = 0 AND ED.FgEnvioCorregido = 0
				INNER JOIN Tramite.ExpedienteDocumentoOrigen EDO WITH (NOLOCK) ON ED.IdExpedienteDocumento=EDO.IdExpedienteDocumento and EDO.EstadoAuditoria=E.EstadoAuditoria
				LEFT JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD WITH (NOLOCK) ON EDOD.IdExpedienteDocumentoOrigen = EDO.IdExpedienteDocumentoOrigen and EDOD.EstadoAuditoria=E.EstadoAuditoria
			WHERE E.EstadoAuditoria=1
				AND E.ExpedienteAnulado=0
				AND EDOD.IdExpedienteDocumentoOrigenDestino Is Null
			OPTION (MAXDOP 2)
