...
CREATE PROCEDURE [Tramite].[paListarExpedientePendienteEspecialistaPorRecibir_CALIDAD]
	@pConFiltroFecha bit,
	...
	BEGIN TRY
	 ...
	 begin
		...
		BEGIN
			...
			INNER JOIN Tramite.ExpedienteDocumento ED  WITH (NOLOCK) ON E.IdExpediente=ED.IdExpediente AND ED.EstadoAuditoria=1 AND E.ExpedienteAnulado=0 AND E.EstadoAuditoria=1 AND ED.FgEnEsperaFirmaDigital=0
			...
			--YEAR(CONVERT(DATETIME,edod.FechaDestino)) = @pIdPeriodo
			--AND
			...
			AND (E.NumeroExpediente =  CASE WHEN @pBusquedaGeneral<>'' OR @pBusquedaGeneral IS NOT NULL THEN  CASE WHEN ISNUMERIC(@pBusquedaGeneral)=1 THEN @pBusquedaGeneral ELSE E.NumeroExpediente END ELSE E.NumeroExpediente END)
		...
		FM.FechaMovimiento --Tramite.funObtenerFechaMovimientoEnExpedienteEspecialista(E.IdExpediente,@vIdArea,@vIdCargo,@pIdPersona,@pIdCatalogoSituacionMovimientoDestino) FechaMovimiento
		FROM
		Tramite.Expediente E WITH (NOLOCK)
		...
		CROSS APPLY(
			...
			WHERE
			EDOD.IdAreaDestino=@vIdArea AND
		...
		SELECT
		convert(BIT,case when PA1.Cant>0 then 0 --Tramite.funParaAnularEspecialista(E.IdExpediente,@pIdPersona,@vIdEmpresa,@vIdArea,@vIdCargo) EsParaAnular,
		     when PA2.Cant>0 then 1 else 0 end) EsParaAnular,
		isnull(DP.DiasPendiente,0) DiasPendiente, --Tramite.funObtenerDiasPendienteEspecislista(E.IdExpediente,@pIdPersona, @vIdEmpresa,@vIdArea,@vIdCargo,@pIdCatalogoSituacionMovimientoDestino) DiasPendiente,
		isnull(NP.NombrePersonaOrigen,'') NombrePersonaOrigen, --Tramite.funObtenerAreaEmisorEspecialista(E.IdExpediente,@vIdArea,@vIdCargo,@pIdPersona,@pIdCatalogoSituacionMovimientoDestino) NombrePersonaOrigen,
		...
		CROSS APPLY(
			...
			INNER JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD WITH (NOLOCK) ON EDOD.IdExpedienteDocumentoOrigen=EDO.IdExpedienteDocumentoOrigen AND EDO.EstadoAuditoria=1
			WHERE E1.IdExpediente=E.IdExpediente and EDOD.EsInicial=1 and ed.EsVinculado=0  and edod.IdCatalogoSituacionMovimientoDestino<>4 and COALESCE(EDOD.FechaDestinoRecepciona,'')='' AND E1.EstadoAuditoria=1 and edo.IdAreaOrigen=@vIdArea AND  EDO.IdPersonaOrigen=@pIdPersona
		...
		CROSS APPLY(
			...
			INNER JOIN Tramite.ExpedienteDocumentoOrigen EDO WITH (NOLOCK) ON EDO.IdExpedienteDocumento=ED.IdExpedienteDocumento AND ED.EstadoAuditoria=1
			INNER JOIN Tramite.ExpedienteDocumentoOrigenDestino EDOD WITH (NOLOCK) ON EDOD.IdExpedienteDocumentoOrigen=EDO.IdExpedienteDocumentoOrigen AND EDO.EstadoAuditoria=1
			WHERE E1.IdExpediente=E.IdExpediente and EDOD.EsInicial=1 and ed.EsVinculado=0  and COALESCE(EDOD.FechaDestinoRecepciona,'')='' AND E1.EstadoAuditoria=1 and edo.IdAreaOrigen=@vIdArea AND  EDO.IdPersonaOrigen=@pIdPersona
				AND  EDO.IdAreaOrigen= @vIdArea
				AND  EDO.IdCargoOrigen=@vIdCargo
				AND  EDO.IdempresaOrigen=@vIdEmpresa
		) PA2
		OUTER APPLY(
			SELECT
			top 1 CASE WHEN  COALESCE(EDOD.FechaDestinoRecepciona,'')='' THEN CASE WHEN DATEDIFF(DAY,CONVERT(DATE, EDO.FechaOrigen),GETDATE())<0 then 0 ELSE DATEDIFF(DAY,CONVERT(DATE, EDO.FechaOrigen),GETDATE()) END ELSE 0 END DiasPendiente
			FROM
			Tramite.ExpedienteDocumento ED  WITH (NOLOCK)
			INNER JOIN Tramite.ExpedienteDocumentoOrigen EDO WITH (NOLOCK) ON ED.IdExpedienteDocumento=EDO.IdExpedienteDocumento AND ED.EstadoAuditoria=1
			INNER JOIN  Tramite.ExpedienteDocumentoOrigenDestino EDOD WITH (NOLOCK) ON EDO.IdExpedienteDocumentoOrigen=EDOD.IdExpedienteDocumentoOrigen AND EDO.EstadoAuditoria=1 AND EDOD.EstadoAuditoria=1
			...
			EDOD.IdCatalogoSituacionMovimientoDestino  =@pIdCatalogoSituacionMovimientoDestino
			AND Ed.IdExpediente=E.IdExpediente AND EDOD.IdCargoDestino =@vIdCargo AND EDOD.IdEmpresaDestino=@vIdEmpresa and edod.IdAreaDestino=@vIdArea and EDOD.IdPersonaDestino=@pIdPersona
		) DP
		OUTER APPLY(
			select isnull((select STUFF((
			SELECT
		        CASE WHEN COALESCE(EDO.IdempresaOrigen,0)=0 THEN ed.NombreCompletoEmisor  ELSE A.NombreArea END +'; ' --CASE WHEN COALESCE(EDO.IdPersonaOrigen ,0)=0 THEN edo.NombreCompletoOrigen  ELSE A.NombreArea END +'; '
			FROM
			Tramite.ExpedienteDocumento ED WITH (NOLOCK)
			INNER JOIN Tramite.ExpedienteDocumentoOrigen EDO WITH (NOLOCK) ON ED.IdExpedienteDocumento=EDO.IdExpedienteDocumento AND ED.EstadoAuditoria=1
			INNER JOIN  Tramite.ExpedienteDocumentoOrigenDestino EDOD WITH (NOLOCK) ON EDO.IdExpedienteDocumentoOrigen=EDOD.IdExpedienteDocumentoOrigen AND EDO.EstadoAuditoria=1 AND EDOD.EstadoAuditoria=1
		...
		OUTER APPLY(
			SELECT
			TOP 1  case when ED.FgEnEsperaFirmaDigital=1 and Ver.doc=0 then '<label style="font-size:8px">'+CASE WHEN ED.Correlativo=0 THEN  CONCAT( CTD.Descripcion,' ', COALESCE(ED.NumeroDocumento,'')) ELSE COALESCE(ED.NumeroDocumento,'') END+'</label>'
			else '<button type="button" data-toggle="tooltip" title="'+COALESCE(EDOD.MotivoArchivado,'')+'" class="btn ui blue label" onclick="MostrarDocumentoPdfExp('''+ED.RutaArchivoDocumento+''','+CONVERT(VARCHAR,ed.IdExpedienteDocumento) +')"><i style="font-size:16px;" class="fa fa-file-text"></i></button><label style="font-size:8px">'+CASE WHEN ED.Correlativo=0 THEN  CONCAT( CTD.Descripcion,' ', COALESCE(ED.NumeroDocumento,'')) ELSE COALESCE(ED.NumeroDocumento,'') END+'</label>' end NumeroDocumento
			FROM
			Tramite.ExpedienteDocumento ED WITH (NOLOCK)
			INNER JOIN Tramite.ExpedienteDocumentoOrigen EDO WITH (NOLOCK) ON ED.IdExpedienteDocumento=EDO.IdExpedienteDocumento AND ED.EstadoAuditoria=1
			INNER JOIN  Tramite.ExpedienteDocumentoOrigenDestino EDOD WITH (NOLOCK) ON EDO.IdExpedienteDocumentoOrigen=EDOD.IdExpedienteDocumentoOrigen AND EDO.EstadoAuditoria=1 AND EDOD.EstadoAuditoria=1
		...
		--Tramite.funObtenerAreaEmisorEspecialista(E.IdExpediente,@vIdArea,@vIdCargo,@pIdPersona,@pIdCatalogoSituacionMovimientoDestino) NombrePersonaOrigen,
		...
		SELECT
		COUNT(IdExpediente)  from @vTablaExpediente
	 end
	 else
	 begin
		SELECT
