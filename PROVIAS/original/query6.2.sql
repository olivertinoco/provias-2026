...
SET NOCOUNT ON;
    ...
    DECLARE @vFechaIni datetime = TRY_CONVERT(datetime, @pFechaInicio, 103);
    DECLARE @vFechaFin datetime = TRY_CONVERT(datetime, @pFechaFin, 103);
    ...
    (
        ...
        INDEX IX_Fecha NONCLUSTERED (FechaMovimiento DESC, IdExpediente)
    );

    /* Compuerta original: si la busqueda general es texto no numerico, no se lista nada */
    IF ISNUMERIC(@pBusquedaGeneral) = 1 OR @pBusquedaGeneral IS NULL OR @pBusquedaGeneral = ''
    BEGIN
        INSERT INTO @Candidatos (IdExpediente, FechaMovimiento)
        SELECT E.IdExpediente,
               MAX(CONVERT(datetime, EDO.FechaOrigen + ' ' + EDO.HoraOrigen, 103))
        ...
        WHERE E.EstadoAuditoria = 1 AND COALESCE(E.ExpedienteAnulado, 0) = 0
          ...
          AND (@pConFiltroFecha = 0
               OR CONVERT(datetime, EDO.FechaOrigen, 103) BETWEEN @vFechaIni AND @vFechaFin)
          /* busqueda general solo cuando es numerica */
          AND (@vBusquedaNumero IS NULL OR E.NumeroExpediente = @vBusquedaNumero)
          /* semantico "debe existir EDOD" via EXISTS (sin el JOIN que multiplicaba filas) */
          AND EXISTS (SELECT 1
    ...
    SELECT
        ...
        EsPrincipalEnlace          = CAST(CASE WHEN EXISTS (SELECT 1
                                                            FROM Tramite.ExpedienteEnlazado EE WITH (NOLOCK)
                                                            WHERE EE.IdExpediente = P.IdExpediente
                                                              AND EE.EstadoAuditoria = 1)
                                               THEN 1 ELSE 0 END AS BIT),
        CatalogoTipoOrigen         = oi.CatalogoTipoOrigen,   -- 2da columna, mismo valor (shape original)
        P.IdExpediente,
    ...
    (
        SELECT
            ...
            NombreExpediente         = CONCAT(SD.AbreviaturaSerieDocumentalExpediente,
                                              RIGHT(CONCAT('000000', E.NumeroExpediente), 6), '-', E.IdPeriodo),
            NombreCompletoCreador    = CASE WHEN COALESCE(E.NombreCompletoCreador, '') <> ''
                                            THEN E.NombreCompletoCreador ELSE PE.NombreCompleto END,
            E.NumeroExpediente,
            IdExpedienteSeguimiento  = COALESCE(ES.IdExpedienteSeguimiento, 0),
            C.FechaMovimiento
        FROM @Candidatos C
        INNER JOIN Tramite.Expediente E WITH (NOLOCK)
               ON E.IdExpediente = C.IdExpediente
        INNER JOIN Seguridad.Usuario US
               ON US.IdUsuario = E.IdUsuarioCreacionAuditoria
              AND E.EstadoAuditoria = 1 AND COALESCE(E.ExpedienteAnulado, 0) = 0
        ...
        ORDER BY C.FechaMovimiento DESC
    ...
    /*------------------------------------------------------------------------------------------
      Result set 2: total (mismos INNER JOIN que la pagina para respetar el conteo original)
    ------------------------------------------------------------------------------------------*/
    SELECT COUNT(*)
    FROM @Candidatos C
    INNER JOIN Tramite.Expediente E WITH (NOLOCK)
