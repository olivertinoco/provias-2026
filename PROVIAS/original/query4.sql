...
AS
	BEGIN TRY
		...
		BEGIN
			...
			SELECT EDO.IdExpediente,MAX(CONVERT(DATETIME,edo.FechaOrigen +' ' + edo.HoraOrigen)) FechaMovimiento FROM
			Tramite.visExpedienteCompleto EDO
			WHERE
			    EDO.IdPersonaOrigen=@pIdPersona
			AND EDO.IdPeriodo = @pIdPeriodo --Expedientes creados por período
			...
			AND CONVERT(DATETIME,EDO.FechaOrigen) BETWEEN  case when @pConFiltroFecha =1 then @pFechaInicio else EDO.FechaOrigen end and case when @pConFiltroFecha =1 then @pFechaFin else EDO.FechaOrigen end
			AND (EDO.NumeroExpediente =  CASE WHEN @pBusquedaGeneral<>'' OR @pBusquedaGeneral IS NOT NULL THEN  CASE WHEN ISNUMERIC(@pBusquedaGeneral)=1 THEN @pBusquedaGeneral ELSE EDO.NumeroExpediente END ELSE EDO.NumeroExpediente END)
