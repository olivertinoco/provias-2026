...
AS
      SET NOCOUNT ON
	   ...
	   SET LANGUAGE 'SPANISH'
			...
			IF @pIdUsuarioAuditoria in(select IdUsuario from Tramite.[PermisoVisualizacionDocumentos] where IdTipoFormulario=2 and EstadoAuditoria=1 and convert(date, GETDATE() )between convert(date,FechaInicioPersmiso) and convert(date,FechaFinPersmiso)) --Habilitar usuarios
			BEGIN
				SET @vSiPariticipo=1
			END
			ELSE
