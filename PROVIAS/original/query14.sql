...
BEGIN TRY
    ...
    Create Table #vTablaExpediente(IdExpediente BigInt, FechaMovimiento Datetime,EsParaAnular int,DiasPendiente int,NombrePersonaOrigen varchar(max),NumeroDocumento varchar(max),IdExpedienteDocumento int, eNroOrden Int);
    ...
    BEGIN
        ...
        begin
            ...
            INSERT INTO #vTablaExpediente(IdExpediente, FechaMovimiento, EsParaAnular, DiasPendiente, NombrePersonaOrigen, NumeroDocumento, IdExpedienteDocumento, eNroOrden)
            SELECT
                SE.IdExpediente,
                SE.FechaMovimiento,
                0 EsParaAnular,
            ...
            FROM
                (
                    SELECT
                        ...
                        MAX(CASE WHEN COALESCE(EDOD.FechaDestinoRecepciona,'')<>'' THEN DATEDIFF(DAY,CONVERT(DATE, EDOD.FechaDestinoRecepciona),GETDATE()) ELSE 0 end) DiasPendiente,
                    ...
                    FROM
                        ...
                        INNER JOIN General.Cargo CAR WITH (NOLOCK) ON CAR.IdCargo = EDOD.IdCargoDestino AND CAR.IdCatalogoTipoCargo in (32,33,34)
            ...
            FROM
                (
                    SELECT
                        E.IdExpediente,
                        MAX(CONVERT(DATETIME,edod.FechaDestinoEnvia +' ' + edod.HoraDestinoEnvia)) FechaMovimiento,
                        MAX('<button type="button" data-toggle="tooltip" class="btn ui blue label" onclick="MostrarDocumentoPdfExp('''+ED.RutaArchivoDocumento+''','+CONVERT(VARCHAR,ED.IdExpedienteDocumento) +')"><i style="font-size:16px;" class="fa fa-file-text"></i></button><label style="font-size:8px;line-height:13px;padding-top:6px;">'+CASE WHEN ED.Correlativo=0 THEN  CONCAT( CTD.Descripcion,' ', COALESCE(ED.NumeroDocumento,'')) ELSE COALESCE(ED.NumeroDocumento,'') END+'</label>') NumeroDocumento,
                        MAX(ED.IdExpedienteDocumento) IdExpedienteDocumento
                    FROM
				        Tramite.Expediente E WITH (NOLOCK)
				        INNER JOIN Tramite.ExpedienteDocumento ED WITH (NOLOCK) ON  E.IdExpediente=ED.IdExpediente AND ED.EstadoAuditoria=E.EstadoAuditoria
    ...
    select
        ...
        (select STUFF((
                        SELECT
                            '¬'+convert(varchar,tE.EsParaAnular), --EsParaAnular,
                            '|'+convert(varchar,tE.DiasPendiente), --DiasPendiente,
                            '|'+tE.NombrePersonaOrigen, --NombrePersonaOrigen,
                            '|'+replace(tE.NumeroDocumento,'|',''), --NumeroDocumento,
                            '|'+convert(varchar,tE.IdExpedienteDocumento), --IdExpedienteDocumento,
                            ...
                            '|'+case when COALESCE(SFU.RutaArchivoFoto,'') ='' then CASE WHEN COALESCE(PE.Sexo,0)=0 then 'sinfotoH.jpg' else 'sinfotoM.jpg' end else SFU.RutaArchivoFoto end, --RutaFotoPersona,
                            '|'+replace(UPPER(E.AsuntoExpediente),'|',' '), --AsuntoExpediente,
                           ...
                           '|'+convert(varchar,E.NumeroExpediente),
                            ...
                            '|'+isnull(FORMAT(tE.FechaMovimiento, 'dd/MM/yyyy HH:mm'),'') --isnull(convert(varchar,tE.FechaMovimiento,21),'')
                      FROM
                        #vTablaExpediente tE
